...
function logError(prefix, message) {
	...
	console.log(
		`${getCurrentTime()} ${colors.redBright(`${characters} ${prefix}:`)}`,
		message
	);
...
module.exports = {
	...
	warn: function (prefix, message) {
		...
		console.log(
			`${getCurrentTime()} ${colors.yellowBright(`${characters} ${prefix}:`)}`,
			message
		);
	},
	...
	info: function (prefix, message) {
		...
		console.log(
			`${getCurrentTime()} ${colors.greenBright(`${characters} ${prefix}:`)}`,
			message
		);
	},
	...
	success: function (prefix, message) {
		...
		console.log(
			`${getCurrentTime()} ${colors.cyanBright(`${characters} ${prefix}:`)}`,
			message
		);
	},
	...
	master: function (prefix, message) {
		...
		console.log(
			`${getCurrentTime()} ${colors.hex("#eb6734", `${characters} ${prefix}:`)}`,
			message
		);
	},
	...
	dev: (...args) => {
		...
		try {
			...
			let position = at.slice(
				at.indexOf(process.cwd()) + process.cwd().length + 1
			);
